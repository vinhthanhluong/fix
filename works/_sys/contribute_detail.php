							...
							<td>
								...
								<ul class="contribute_list">
									...
									if(strlen(trim($field_data['value']))):
										foreach(explode(',',$field_data['value']) as $key => $val ):
											$val = trim($val);
											$chk = (htmlspecialchars(@$contribute['data'][$id][$field_id]) == $val
														 || ( (!strlen(htmlspecialchars(@$contribute['data'][$id][$field_id]))
														 || (is_array($field_data['value']) && !@in_array($val,$field_data['value'])) ) && $key == 0 ) )
														? 'checked="checked"'
														: '';
									?>
									<li>
								...
								<ul class="contribute_list">
									...
									<?php $chk = (is_array($contribute['data'][$id][$field_id]) && @in_array($val,$contribute['data'][$id][$field_id]))
																? 'checked="checked"'
																: ''; ?>
									<li>
										<label><input type="<?php echo $field_data['type'] ?>" name="data[<?php echo $id; ?>][<?php echo $field_id; ?>][]" value="<?php echo $val ?>" <?php echo $chk ?> />
