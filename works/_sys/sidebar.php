...
<?php if(!is_smart_phone()): $menu = array('category'=>'カテゴリ設定','field'=>'項目の種類','setting'=>'基本設定','import'=>'記事インポート',); ?>
	<ul class="navi_menu2">
		...
		<?php if( !( (is_array($setting['menu']) && @in_array($val,$setting['menu'])) && @$_SESSION['login']['role']!='freesale') ): ?><li class="navi_<?php echo $key ?> <?php echo (SELF == $key || SELF == $key.'_detail')? 'active': '' ?>"><a href="<?php echo SYS_URI.'/'.$key ?>.php"><?php echo $val ?></a></li><?php endif ?>
		<?php endforeach ?>
	</ul>
<?php endif ?>
</div>