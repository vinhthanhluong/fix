...
$elt = 0;
$attcnt = 1;# 複数添付ファイル用のカウンタ
for($cnt=0;$cnt<@elements;$cnt++){
	($elements_id,$num,$name,$type_of_element,$html_size,$html_rows,$html_cols,$html_id,$element_type,$check_type,$on_event,$html_tag_free,$text_min,$text_max,$enable_filetypes,$filesize_min,$filesize_max,$checked_min,$checked_max,$element_valus,$element_text,$html_example,$note,$element_error_message,$must_disp,$default_value,$system_disp_false,$html_tag_free_top,$elements_class,$smartphone_element_type,$confirm_hidden,$return_hidden,$or_disp,$log_hidden) = split(/\t/,$elements[$elt]);
	...
	if($element_type ne "spacer" && !$log_hidden){
		...
		if(-f $check_attached_files){
			...
			$record[$cnt] = $serials_this_num . '_' . $attcnt . '.' . $filename_arr[-1];
			$attcnt ++;
...
$print_html = <<"EOF";
	...
	<link href="modules/logview/print.css" rel="stylesheet" type="text/css" media="print" />
	<link href="modules/logview/logview.css" rel="stylesheet" type="text/css" media="all" />
EOF
