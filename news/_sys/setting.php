        ...
        <select name="setting[plan]">
          ...
          <option value="シンプル" <?php if($setting['plan'] == 'シンプル') echo 'selected = "selected"'; ?>>シンプル</option>
			...
			<tr>
				<th>メニュー消去</th>
				<td>
					<label><input type="checkbox" name="setting[menu][]" value="カテゴリ設定" <?php if(is_array($setting['menu']) && @in_array('カテゴリ設定',$setting['menu'])) echo 'checked = "checked"'; ?>>カテゴリ設定</label>
					<label><input type="checkbox" name="setting[menu][]" value="項目の種類" <?php if(is_array($setting['menu']) && @in_array('項目の種類',$setting['menu'])) echo 'checked = "checked"'; ?>>項目の種類</label>
					<label><input type="checkbox" name="setting[menu][]" value="基本設定" <?php if(is_array($setting['menu']) && @in_array('基本設定',$setting['menu'])) echo 'checked = "checked"'; ?>>基本設定</label>
					<label><input type="checkbox" name="setting[menu][]" value="記事インポート" <?php if(is_array($setting['menu']) && @in_array('記事インポート',$setting['menu'])) echo 'checked = "checked"'; ?>>記事インポート</label>
				</td>
      </tr>
    </table>
